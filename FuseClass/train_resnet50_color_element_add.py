...
from models.feature_extract_model_easy_method_color_element_add import extractmodel
...
class train_cfg:
    ...
    in_channels = 1
    # save
    save_path = r"/root/autodl-tmp/weights/weights_resnet50_color_element_add_gate_unit"
...
def train(local_rank):
    ...
    if cfg.dataset_type == "MNIST":
        ...
        train_path = r"./test/train.txt"
        val_path = r"./test/val.txt"
        train_images_path, train_images_label = load_data(train_path)
        val_images_path, val_images_label = load_data(val_path)
    ...
    # 使用linear下降策略，因为cosine策略训练效果太好，不好判断性能是否提高
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        with tqdm_auto(range(len(train_dataloader))) as pbar:
            ...
            str_log = "rank: {} TrainLoss: {}, Epoch: {} Lr: {}, accu: {}".format(local_rank, loss, epoch, lr_scheduler.get_last_lr()[0], accu_num.item() / sample_num)
            logging.info(str_log)
            train_accu_list.append(accu_num.item() / sample_num)
        ...
        with tqdm_auto(range(len(val_dataloader))) as pbar:
            ...
            logging.info(str_log)
            val_accu_list.append(accu_num.item() / sample_num)
            val_loss_list.append(loss)

        # 保存断点数据
        if (epoch + 1) % 1 == 0:
            # save_path = r"/root/autodl-tmp/weights/weights_resnet50_element_add_element_add"
    ...
    plt_result(np.array(list(range(cfg.epochs))), np.array(train_loss_list), "Train loss")
    plt_result(np.array(list(range(cfg.epochs))), np.array(val_loss_list), "Val loss")
    plt_result(np.array(list(range(cfg.epochs))), np.array(train_accu_list), "Train accu")
    plt_result(np.array(list(range(cfg.epochs))), np.array(val_accu_list), "Val accu")
