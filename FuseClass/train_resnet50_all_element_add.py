...
from models.feature_extract_model_easy_method_all_element_add import extractmodel
...
class train_cfg:
    ...
    in_channels = 1
    # save
    save_path = r"/root/autodl-tmp/weights/weights_resnet50_all_element_add_gate_unit"
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        with tqdm_auto(range(len(train_dataloader))) as pbar:
            ...
            for global_step, batch in zip(pbar, train_dataloader):
                ...
                img, label, all_img = batch
                sample_num += img.shape[0]

                img = img.cuda(local_rank)
                label = label.cuda(local_rank)
                all_img = all_img.cuda(local_rank)
        ...
        # 保存断点数据
        if (epoch + 1) % 1 == 0:
            # save_path = r"/root/autodl-tmp/weights/weights_resnet50_all_features_weighted_sum"
