...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        all_img = torch.cat((cannymap, hsv_img, hls_img, lab_img), dim=0)
        
        # 提取hog特征
        # hog_features = calculate_hog(rgb_img)
        # print(hog_features, hog_features.shape)

        return img, label, all_img
    ...
    def collate_fn(batch):
        ...
        images, labels, all_img = tuple(zip(*batch))
        # 将这批图片堆叠起来
        ...
        # 将canny、hsv、hls、lab特征堆叠起来
        all_features = torch.stack(all_img, dim=0)
