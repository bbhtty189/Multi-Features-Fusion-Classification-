...
class extractmodel(nn.Module):
    def __init__(self, cfg):
        ...
        self.extract_color = nn.Sequential(
            nn.Conv2d(9, (self.total_dim // 16), kernel_size=3, stride=2, padding=1),
        ...
        self.weight_color = nn.Parameter(torch.ones(self.total_dim, 7, 7))
        # self.weight_hog = nn.Parameter(torch.ones(2048))

        # gate unit
        self.cond_gate = nn.Linear(self.total_dim, self.total_dim)
        self.color_gate = nn.Linear(self.total_dim, self.total_dim)
    ...
    def forward(self, img, color):
        ...
        color_features = self.extract_color(color)

        fuse_features = None
        if self.easy_method == "cat":
            # fuse color on channels
            fuse_features = torch.cat((cond, color_features), dim=1)
            ...
            # fuse color by adding element one by one.
            fuse_features = cond + color_features
            fuse_features = self.pooling(fuse_features)
            # (B,C,1,1) -> (B,C)
            fuse_features = torch.flatten(fuse_features, 1)
            ...
            # fuse color by multiply weights
            ...
            weighted_color = torch.mul(color_features, self.weight_color)
            # 求和池化 (B,C,H,W) -> (B,C,N) -> (B,C,1)
            weighted_color = weighted_color.view(weighted_color.shape[0], weighted_color.shape[1], -1).contiguous()
            weighted_color = torch.sum(weighted_color, dim=2)

            # fuse_features = torch.cat((weighted_cond, weighted_color), dim=1)
            # fuse_features = fuse_features.view(fuse_features.shape[0], fuse_features.shape[1], 1, 1).contiguous()
            # fuse_features = self.dim_reduction(fuse_features)

            fuse_features = weighted_cond + weighted_color
            ...
            # (B, 2048, ,7, 7) -> (B, 2048, 1, 1) -> (B, 2048)
            color_pooling = self.pooling(color_features)
            color_pooling = torch.flatten(color_pooling, 1)

            # cond gate unit
            cond_weights = self.cond_gate(cond_pooling)
            color_weights = self.color_gate(color_pooling)
            cond_weights = cond_weights.view(cond_weights.shape[0], cond_weights.shape[1], 1, 1).contiguous()
            color_weights = color_weights.view(color_weights.shape[0], color_weights.shape[1], 1, 1).contiguous()

            # print(cond.shape)
            # print(color_features.shape)
            # print(cond_weights.shape)
            # print(color_weights.shape)

            # fuse_features = torch.cat((cond_weights * cond, color_weights * color_features), dim=1)
            # fuse_features = self.dim_reduction(fuse_features)
            # fuse_features = self.pooling(fuse_features)

            fuse_features = cond_weights * cond + color_weights * color_features
            fuse_features = self.pooling(fuse_features)
...
if __name__ == "__main__":
    ...
    from my_dataset_color import MyDataSet
    ...
    for img, label, color in test_dataloader:
