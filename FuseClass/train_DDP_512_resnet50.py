...
from utils import plt_result, read_data
...
def train(local_rank):
        ...
        for k in list(weights_dict.keys()):
            if k in layers_False:
                if np.shape(model_dict[k]) == np.shape(weights_dict[k]):
                    ...
                    del weights_dict[k]
        print(model.res.load_state_dict(weights_dict, strict=False))

    #  断点处重启训练
    if cfg.resume != "" and local_rank == 0:
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        # 保存断点数据
        if (epoch + 1) % 1 == 0 and local_rank == 0:
